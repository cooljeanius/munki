...
class MSUAppDelegate(NSObject):
    ...
    def applicationDidFinishLaunching_(self, sender):
        NSLog("Managed Software Update finished launching.")
        ...
        runmode = NSUserDefaults.standardUserDefaults().stringForKey_(
            "mode"
    ...
    def _sortUpdateList(self, l):
        ...
        def sort_lambda(i):
            return (i.get("display_name") or i.get("name")).lower()
    ...
    def updateAvailableUpdates(self):
        NSLog("Managed Software Update got update notification")
    ...
    def munkiStatusSessionEnded_(self, socketSessionResult):
        ...
        alertMessageText = NSLocalizedString("Update check failed", None)
        if self.managedsoftwareupdate_task == "installwithnologout":
            alertMessageText = NSLocalizedString("Install session failed", None)
        ...
        if socketSessionResult == -1:
            ...
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                alertMessageText,
                NSLocalizedString("Quit", None),
                objc.nil,
                objc.nil,
                NSLocalizedString(
                    "There is a configuration problem with the managed software installer. The process ended unexpectedly. Contact your systems administrator.",
                    None,
                ),
            )
            alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
                self.mainWindowController.theWindow,
                self,
                self.quitAlertDidEnd_returnCode_contextInfo_,
                objc.nil,
            )
            return
        ...
        if self.managedsoftwareupdate_task == "installwithnologout":
            ...
            if lastCheckResult == 0:
                ...
                alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                    NSLocalizedString("Cannot check for updates", None),
                    NSLocalizedString("Quit", None),
                    objc.nil,
                    objc.nil,
                    NSLocalizedString(
    ...
    def noUpdatesAlert(self):
        if self._optionalInstalls:
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Your software is up to date.", None),
                NSLocalizedString("Quit", None),
                NSLocalizedString("Optional software...", None),
                objc.nil,
                NSLocalizedString(
                    "There is no new software for your computer at this time.", None
                ),
            )
            alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
                self.mainWindowController.theWindow,
    ...
    def checkForUpdates(self):
        ...
        if result == 0:
            ...
            self.mainWindowController.theWindow.makeKeyAndOrderFront_(self)
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Update check failed", None),
                NSLocalizedString("Quit", None),
                objc.nil,
                objc.nil,
                NSLocalizedString(
                    "There is a configuration problem with the managed software installer. Could not start the update check process. Contact your systems administrator.",
                    None,
                ),
            )
            alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
            ...
            for update in updatelist:
                ...
                if force_install_after_date:
                    ...
                    forced_date_text = NSLocalizedString(
                        "This item must be installed by ", None
                    )
            ...
            if installinfo.get("removals"):
                ...
                for item in removallist:
                    ...
                    if showRemovalDetail:
                        ...
                        ) + NSLocalizedString(" (will be removed)", None)
                        item["description"] = NSLocalizedString(
                            "This item will be removed.", None
                        )
                        updatelist.append(item)
                if not showRemovalDetail:
                    row = {}
                    row["display_name"] = NSLocalizedString("Software removals", None)
                    row["version"] = ""
                    row["description"] = NSLocalizedString(
                        "Scheduled removal of managed software.", None
                    )
                    if restartNeeded:
        ...
        for item in self._optionalInstalls:
            ...
            if row["installed"]:
                if item.get("needs_update"):
                    status = NSLocalizedString("Update available", None)
                else:
                    row["size"] = "-"
                    status = NSLocalizedString("Installed", None)
                if item.get("will_be_removed"):
                    status = NSLocalizedString("Will be removed", None)
                elif not item.get("uninstallable"):
                    status = NSLocalizedString("Not removable", None)
                    row["enabled"] = objc.NO
            else:
                status = "Not installed"
                if item.get("will_be_installed"):
                    status = NSLocalizedString("Will be installed", None)
                elif item.get("note"):
    ...
    def buildUpdateTableData(self):
        ...
        for item in self._listofupdates:
            ...
            if item.get("force_install_after_date"):
                ...
                item.get("RestartAction") == "RequireLogout"
    ...
    def forcedLogoutWarning(self, notification_obj):
        ...
        if time_til_logout > 55:
            ...
            infoText = (
                NSLocalizedString("A logout will be forced at approximately %s.", None)
                % deadline_str
                + moreText
            )
            ...
            infoText = (
                NSLocalizedString(
                    "A logout will be forced in less than %s minutes.", None
                )
                % time_til_logout
                + moreText
            )
        ...
        self._force_warning_logout_btn = NSLocalizedString(
            "Logout and update now", None
        )
        self._force_warning_ok_btn = NSLocalizedString("OK", None)
        ...
        alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
            NSLocalizedString("Forced Logout for Mandatory Install", None),
        ...
        alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
            self.mainWindowController.theWindow,
            self,
            self.forceLogoutWarningDidEnd_returnCode_contextInfo_,
            objc.nil,
        )
    ...
    def laterBtnClicked(self):
        if munki.thereAreUpdatesToBeForcedSoon():
            ...
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Manadatory Updates Pending", None),
                NSLocalizedString("Show updates", None),
                NSLocalizedString("Update later", None),
                objc.nil,
                infoText,
            )
            self._currentAlert = alert
            alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
                self.mainWindowController.theWindow,
                self,
                self.confirmLaterAlertDidEnd_returnCode_contextInfo_,
                objc.nil,
            )
        else:
    ...
    def confirmInstallUpdates(self):
        ...
        if len(munki.currentGUIusers()) > 1:
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Other users logged in", None),
                NSLocalizedString("Cancel", None),
                objc.nil,
                objc.nil,
                NSLocalizedString(
            ...
            alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
                self.mainWindowController.theWindow,
                self,
                self.multipleUserAlertDidEnd_returnCode_contextInfo_,
                objc.nil,
            )
        elif self.restart_required:
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Restart Required", None),
                NSLocalizedString("Logout and update", None),
                NSLocalizedString("Cancel", None),
                objc.nil,
                NSLocalizedString(
                    "A restart is required after updating. Please be patient as there may be a short delay at the login window. Logout and update now?",
                    None,
                ),
            )
            self._currentAlert = alert
            alert.beginSheetModalForWindow_modalDelegate_didEndSelector_contextInfo_(
                self.mainWindowController.theWindow,
                self,
                self.logoutAlertDidEnd_returnCode_contextInfo_,
                objc.nil,
            )
        elif self.logout_required or munki.installRequiresLogout():
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Logout Required", None),
                NSLocalizedString("Logout and update", None),
                NSLocalizedString("Cancel", None),
                objc.nil,
                NSLocalizedString(
                    "A logout is required before updating. Please be patient as there may be a short delay at the login window. Logout and update now?",
                    None,
                ),
            )
            self._currentAlert = alert
            ...
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Logout Recommended", None),
                NSLocalizedString("Logout and update", None),
                NSLocalizedString("Cancel", None),
                NSLocalizedString("Update without logging out", None),
                NSLocalizedString(
                    "A logout is recommended before updating. Please be patient as there may be a short delay at the login window. Logout and update now?",
                    None,
                ),
            )
            self._currentAlert = alert
    ...
    def alertIfBlockingAppsRunning(self):
        ...
        if running_apps:
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString("Conflicting applications running", None),
                NSLocalizedString("OK", None),
                objc.nil,
                objc.nil,
                NSLocalizedString(
    ...
    def alertIfRunnningOnBattery(self):
        ...
        if (
            power_info.get("PowerSource") == "Battery Power"
            and power_info.get("BatteryCharge", 0) < 50
        ):
            alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
                NSLocalizedString(
                    "Your computer is not connected to a power source.", None
                ),
                NSLocalizedString("Continue", None),
                NSLocalizedString("Cancel", None),
                objc.nil,
                NSLocalizedString(
                    "For best results, you should connect your computer to a power source before updating. Are you sure you want to continue the update?",
                    None,
                ),
            )
    ...
    def installSessionErrorAlert(self):
        alert = NSAlert.alertWithMessageText_defaultButton_alternateButton_otherButton_informativeTextWithFormat_(
            NSLocalizedString("Cannot start installation session", None),
            NSLocalizedString("Quit", None),
            objc.nil,
            objc.nil,
            NSLocalizedString(
                "There is a configuration problem with the managed software installer. Could not start the install session. Contact your systems administrator.",
                None,
            ),
        )
