...
class MSUOptionalInstallsViewController(NSViewController):
    ...
    _EMPTYOPTIONALLIST = NSArray.arrayWithArray_(
        [
            {
                "installed": NO,
                "managed": NO,
    ...
    def searchFilterChanged_(self, sender):
        ...
        self.setFilteredlist_(
            [
                item
                for item in self._optionallist
                if filterString in item["name"].lower()
                or filterString in item["description"].lower()
            ]
        )
            ...
            if row["managed"] == row["original_managed"]:
                ...
                if row["managed"]:
                    ...
                    if row["original_status"] == NSLocalizedString(
                        ...
                        row["status"] = NSLocalizedString("Will be installed", None)
                    elif row["original_status"] == NSLocalizedString(
                        "Update available", None
                    ):
                        row["status"] = NSLocalizedString("Will be updated", None)
                    elif row["original_status"] == NSLocalizedString(
                        "Will be removed", None
                    ):
                        row["status"] = NSLocalizedString("Will not be removed", None)
                else:
                    # not row['managed']
                    if row["original_status"] == NSLocalizedString("Installed", None):
                        row["status"] = NSLocalizedString("Will be removed", None)
                    elif row["original_status"] == NSLocalizedString(
                        "Will be installed", None
                    ):
                        row["status"] = NSLocalizedString("Will not be installed", None)
