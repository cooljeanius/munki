...
def mountAdobeDmg(dmgpath):
    ...
    proc = subprocess.Popen(
        ["/usr/bin/hdiutil", "attach", dmgpath, "-nobrowse", "-noverify", "-plist"],
        bufsize=1,
    ...
    if err:
        munkicommon.display_error("Error {} mounting {}.".format(err, dmgname))
    if pliststr:
...
def getCS5uninstallXML(optionXMLfile):
    ...
    if DeploymentInfo:
        ...
        if DeploymentUninstall:
            deploymentData = DeploymentUninstall[0].getElementsByTagName("Deployment")
            if deploymentData:
...
def getCS5mediaSignature(dirpath):
    ...
    if os.path.exists(setupxml) and os.path.isfile(setupxml):
        ...
        if setupElements:
            mediaSignatureElements = setupElements[0].getElementsByTagName(
                "mediaSignature"
            )
            ...
            for item in munkicommon.listdir(path):
                ...
                if payloadinfo:
                    ...
                    if (driverfolder and item == driverfolder) or (
                        mediaSignature and payloadinfo["AdobeCode"] == mediaSignature
                    ):
...
def getAdobeInstallProgressInfo(previous_completedpayloads, previous_payloadname):
        ...
        if pid:
            if pid not in secondsToLive:
                ...
                if secondsToLive[pid] == 0:
                    ...
                    munkicommon.log("Killing PID {}: {}".format(pid, procname))
                    try:
                        os.kill(int(pid), 9)
                    except OSError:
                        pass
...
def runAdobeInstallTool(cmd, number_of_payloads=0, killAdobeAIR=False):
    ...
    while proc.poll() == None:
        ...
        if killAdobeAIR:
            if (
                not munkicommon.getconsoleuser()
                or munkicommon.getconsoleuser() == "loginwindow"
            ):
    ...
    if retcode != 0 and retcode != 8:
        munkicommon.display_error(
            "Adobe Setup error: {}: {}".format(retcode, adobeSetupError(retcode))
        )
    else:
        if munkicommon.munkistatusoutput:
...
def runAdobeCS5AAMEEInstall(dmgpath):
    ...
    if deploymentmanager:
        ...
        for item in munkicommon.listdir(realsetupdir):
            os.symlink(
                os.path.join(realsetupdir, item), os.path.join(tmpsetupdir, item)
            )
        ...
        cmd.extend(
            [
                deploymentmanager,
                "--optXMLPath=%s" % optionXMLfile,
                "--setupBasePath=%s" % tmpdir,
                "--installDirPath=/",
                "--mode=install",
            ]
        )
...
def runAdobeCS5PatchInstaller(dmgpath, copylocal=False):
    ...
    if mountpoints:
        if copylocal:
            ...
            munkicommon.unmountdmg(mountpoints[0])
            if retcode:
                munkicommon.display_error("Error copying items from %s" % dmgpath)
                return -1
        ...
        if patchinstaller:
            ...
            retcode = runAdobeInstallTool(install_cmd, number_of_payloads)
        else:
            munkicommon.display_error(
...
def runAdobeUberTool(dmgpath, pkgname="", uninstalling=False):
    ...
    if mountpoints:
        ...
        if os.path.exists(ubertool):
            ...
            if uninstalling:
                action = "Uninstalling"
            munkicommon.display_status_major("{} {}".format(action, packagename))
            if munkicommon.munkistatusoutput:
...
def updateAcrobatPro(dmgpath):
    ...
    if not pathToAcrobatPatchApp:
        munkicommon.display_error("No Acrobat Patch app at %s" % pathToAcrobatPatchApp)
        munkicommon.unmountdmg(installroot)
        return -1

    # some values needed by the patching script
    resourcesDir = os.path.join(pathToAcrobatPatchApp, "Contents", "Resources")
    ...
    if os.path.exists(appListFile):
        fileobj = open(appListFile, buffering=1)
        if fileobj:
    ...
    for line in appList:
        ...
        if os.path.exists(pathname):
            ...
            candidates = [
                item
                for item in munkicommon.getAppData()
        ...
        if len(candidates) > 1:
            munkicommon.display_error(
                "Cannot patch %s because we found "
        ...
        retcode = proc.poll()
        if retcode != 0:
            munkicommon.display_error("Error patching {}: {}".format(appname, retcode))
            break
        else:
...
def getAdobeCatalogInfo(mountpoint, pkgname=""):
    ...
    if deploymentmanager:
        ...
        if cataloginfo:
            ...
            cataloginfo["adobe_install_info"] = getAdobeInstallInfo(installdir=dirpath)
            mediasignature = cataloginfo["adobe_install_info"].get("media_signature")
            if mediasignature:
...
def doAdobeRemoval(item):
    ...
    if "uninstaller_item" in item:
        ...
        if not os.path.exists(itempath):
            munkicommon.display_error(
                "%s package for %s was "
                "missing from the cache." % (uninstallmethod, item["name"])
            )
            return -1
